...
public:
		...
		shader(vertex_shader, fragment_shader)
	{
		...
		std::bernoulli_distribution bernoulii(0.5);

		// prevLeft, prevRight, nextLeft, nextRight
		std::vector<float> vertices;
		std::vector<unsigned int> indices;
		vertices.reserve((lanes.size() + 1) * 3);
		indices.reserve(elnum);
		treePos.reserve(lanes.size());

		unsigned int offset = 0;
		float height = 0.f, altitueScale = glm::tan(glm::radians(15.0f));
		...
		push_vec3(vertices, nextLeft, height);
		push_vec3(vertices, nextRight, height);

		sDir = Dir * length;
		nextLeft = sDir + orthoDir;
		nextRight = sDir - orthoDir;

		auto b = bernoulii(gen);
		treePos.push_back(prevPoint + sDir * 0.5f + ((float)b *2 - 1.f) * orthoDir * 2.f + glm::vec2(dist(gen), dist(gen)));
		prevPoint += sDir;

		height += length * altitueScale;
	...
	void draw(Camera& camera, glm::mat4& projection, Model* tree = nullptr, Shader* treeShader = nullptr)
	{
		...
		{
			treeShader->use();
			glm::mat4 mvp_tree, model;
			treeShader->setVec3("camPos", camera.Position);
			int cnt=0;
			for (glm::vec2& pos : treePos) 
			{
				model = glm::translate(glm::mat4(1.f), glm::vec3(pos.x, 0.f, pos.y));
				mvp_tree = mvp * model;
				treeShader->setMat4("mvp", mvp_tree);
				treeShader->setMat3("normalMatrix", glm::transpose(glm::inverse(glm::mat3(model))));
...
private:
	void push_vec3(std::vector<float>& vertices, glm::vec2 vec, float h)
	{
		vertices.insert(vertices.end(), {vec.x, h, vec.y});
	}
