...
enum AnimTextureType { DIFFUSE, NORMAL, SPECULAR, HEIGHT };

struct AnimTextureOverride
{
...
{
public:
	vector<AnimTextureOverride> overrides;
	...
	AnimModel(string path, vector<AnimTextureOverride> texOver, bool gamma = false) : overrides(texOver), gammaCorrection(gamma)
	{
	...
	AnimModel(string path, bool gamma = false) : gammaCorrection(gamma)
	{
		Assimp::Importer importer;
		const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
...
private:
	...
	void processNode(aiNode* node, const aiScene* scene)
	{
		...
		{
			unsigned int meshIndex = meshes.size(); // 현재 메쉬 인덱스
			meshes.push_back(processMesh(scene->mMeshes[node->mMeshes[i]], scene, meshIndex));
		}
	...
	ObjectMesh processMesh(aiMesh* mesh, const aiScene* scene, unsigned int meshIndex)
	{
		...
		// Load mesh materials (no override logic anymore)
		...
		for (const auto& ovr : overrides) {
			if (ovr.meshIndex == meshIndex) {
				switch (ovr.type) {
				case DIFFUSE:
					diffuseMaps.push_back(loadCustomTexture(ovr.path));
					overrideDiffuse = true;
					break;
				case NORMAL:
					normalMaps.push_back(loadCustomTexture(ovr.path));
					overrideNormal = true;
					break;
				case SPECULAR:
					specularMaps.push_back(loadCustomTexture(ovr.path));
		...
		if (!overrideNormal)
			normalMaps.push_back(loadMaterialTextures(material, aiTextureType_NORMALS));
