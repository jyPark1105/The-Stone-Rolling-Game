...
public:
    ...
    void setInstanced(unsigned int amount, glm::mat4* modelMatrices)
    {
        for(unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].setInstanced(amount, modelMatrices);
    }

    static glm::vec3 getAABBSize(const string& modelPath) {
        Assimp::Importer importer;
        const aiScene* scene = importer.ReadFile(modelPath,
            aiProcess_Triangulate | aiProcess_JoinIdenticalVertices);

        if (!scene || !scene->mRootNode) {
            std::cerr << "[OurModel::getAABBSize] Failed to load scene for AABB: " << modelPath << std::endl;
